...
export default function NameFortunePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.lastName || !form.firstName || !form.birthdate || !form.gender) return;
    setLoading(true);
    try {
      const res = await fetch("/api/fortune", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, type: "name", method: "姓名判断" }),
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-56px)] px-4 py-12">
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">姓名判断</h1>
        <p className="text-white/70">名前に秘められた運命をAIが読み解きます</p>
      </div>

      <Card className="w-full max-w-md">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="grid grid-cols-2 gap-3">
            <InputField label="姓（漢字） *" placeholder="山田" required value={form.lastName} autoComplete="family-name"
              onChange={(e) => setForm({ ...form, lastName: e.target.value })} />
            <InputField label="名（漢字） *" placeholder="太郎" required value={form.firstName} autoComplete="given-name"
